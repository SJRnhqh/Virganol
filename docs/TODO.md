...
- [x] Common store `provider_id` parameter removed — `load_settings` / `save_settings` / `open_store` / `load_all_providers` no longer accept `ProviderId`; common store layer fully agnostic to caller identity
