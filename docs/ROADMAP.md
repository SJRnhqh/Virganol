...
``` txt
┌─────────────────────────────────────┐
│ 🔗 localhost:11434               +  │ ← 顶部工具栏（连接信息 + 全选）
├─────────────────────────────────────┤
│ - deepseek-chat                     │ ← 已启用（减号）
│ + qwen-2.5                          │ ← 未启用（加号）
└─────────────────────────────────────┘
```
...
- 符号表示"点击后的操作"：`+` = 启用/全选，`-` = 禁用/全不选
- 极简风格：纯符号，无边框、无背景、无动画
- 符合羊皮纸风格：文本符号更复古

**🚨 紧急待办**：

- [ ] 实现 `ProviderCardActions` 组件（Reset 功能）— Reset 按钮已从 ConnectedPanel 移除，
