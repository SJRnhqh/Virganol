...
- `Io` / `Serde` — 持久化读写
- `UnsupportedProvider` — 未注册的 provider
- `LifecycleEventEmit` — 事件推送失败
- `LifecycleTaskJoin` — 并发任务异常（panic/cancel）
- `LifecyclePartialFailure` — 生命周期终态汇总

### 🔜 Phase 2：错误 code 与 message 统一管理

- 将 `code()` 返回值从方法内 match 提升为常量或关联常量，便于前后端对齐
